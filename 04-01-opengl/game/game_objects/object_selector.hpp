...
public:
    void update() override
    {
                ...
                pt::key_code::mouse_left))
        {
            ...
            auto mouse_world =
                ptm::glm_vec(get_mouse_pos()) * screen_to_world_m;

            mouse_world /= mouse_world.w;

            auto bt_pos =
                btVector3{ mouse_world.x, mouse_world.y, mouse_world.z };

            btCollisionWorld::ClosestRayResultCallback rayCallBack(
                bt_pos, bt_dir * 100);

            get_engine().get_dynamics_world()->rayTest(
                bt_pos, bt_dir * 100, rayCallBack);
    ...
    ptm::vec4 get_mouse_pos()
    {
        ...
        auto mouse_pos = normalize_screen_coords(x, y, window_size.x, window_size.y);

        std::cout << mouse_pos << std::endl;

        return { mouse_pos.x, mouse_pos.y, -1.f, 1.0f };
    }
