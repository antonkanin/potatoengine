...
void main()
{
    ...
    float layers_count = 64.0;
    float hair_length = 0.3;
    v_position += hair_length * (gl_InstanceID / layers_count) * vec4(normalize(vec3(v_normal)) + u_gravity * gl_InstanceID, 0.0);
    // v_position += (gl_InstanceID / 200.0) * vec4(normalize(vec3(v_normal)) + u_gravity * gl_InstanceID, 0.0);

    float max_value = gl_InstanceID / layers_count;
